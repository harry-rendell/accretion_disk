...
def spectrum_full_disc(self,nu,R_min,R_max):
...
#    coeff = 1
    ...
    if (R_max <= R_mid) & (R_min < R_mid):
        ...
        factor_r1 = (self.x1[R_min:R_mid]**2)[np.newaxis,:,np.newaxis,np.newaxis]
        factor_dist1 = (np.exp(np.divide.outer(nu,dT_rp[R_min:R_mid,:,:]))-1)**-1
        B_inner = (a1 * factor_nu * factor_r1 * factor_dist1)

        factor_r2 = (self.x1[R_mid:R_max]**2)[np.newaxis,:,np.newaxis,np.newaxis]
