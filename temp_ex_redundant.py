...
def load(i,wdir,offset):
    ...
    q_rtp = D.bx1[r_min:r_max,t_min:t_max,:]*D.bx3[r_min:r_max,t_min:t_max,:]
    return q_rtp
...
#    dT_r = abs(np.array([ q_lower, q_upper, (q_lower+q_upper)*0.5 ]) * (A / B) * (x1 ** (-0.5)) * isigma)**0.25
    q_rp = np.tensordot(q_rtp,sine,axes=(1,0))/170
    dT_rp = abs( 1.5 * q_rp * (x1**(-0.5) * (A / B))[:,np.newaxis] * isigma)**0.25
    return dT_rp
...
offset = 239
n = 0
filename = 'dT_extended_50'
dT = np.zeros(((r_max-r_min),128,n))
